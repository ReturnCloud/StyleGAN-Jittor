...
if __name__ == '__main__':
    ...
    # parser.add_argument('--path', type=str, default='./exp1/checkpoint/030000.model', help='path to checkpoint file')
    parser.add_argument('--exp_path', default='./exp', help='path of experiment')

    args = parser.parse_args()
    args.path = f'./{exp_path}/checkpoint/040000.model'
    ...
    # np.save('./tmp.npy', np.array(img))

    jt.save_image(img, f'./{exp_path}/style_mixing/sample.png', nrow=args.n_col, normalize=True, range=(-1, 1))

    for j in range(20):
        img = style_mixing(generator, step, mean_style, args.n_col, args.n_row)
        jt.save_image(
            img, f'./{exp_path}/style_mixing/sample_mixing_{j}.png', nrow=args.n_col + 1, normalize=True, range=(-1, 1)
        )